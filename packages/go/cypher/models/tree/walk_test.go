...
type walker tree.HierarchicalVisitor[cypher.Expression]
...
func TestWalk(t *testing.T) {
	...
	for _, testCase := range test.LoadFixture(t, test.PositiveTestCases).RunnableCases() {
		...
		if testCase.Type == test.TypeStringMatch {
			...
			require.Nil(t, tree.WalkCypher(queryModel, tree.NewComposableHierarchicalVisitor[cypher.SyntaxNode]()))
		}
	}
}
