...
func formatNode(builder OutputBuilder, rootExpr pgsql.SyntaxNode) error {
	...
	for len(exprStack) > 0 {
		...
		case pgsql.Parameter:
			exprStack = append(exprStack, typedNextExpr.Identifier)
...
func Expression(expression pgsql.SyntaxNode) (string, error) {
	var (
		builder = NewOutputBuilder()
		err     = formatNode(builder, expression)
	)
...
func formatFromClauses(builder OutputBuilder, fromClauses []pgsql.FromClause) error {
	for idx, fromClause := range fromClauses {
		...
		if err := formatNode(builder, fromClause.Relation); err != nil {
			return err
		}
...
func formatDeleteStatement(builder OutputBuilder, delete pgsql.Delete) error {
	...
	if err := formatNode(builder, delete.Table); err != nil {
		return err
	}
	...
	if delete.Where != nil {
		...
		if err := formatNode(builder, delete.Where); err != nil {
			return err
		}
	}
...
func Statement(statement pgsql.Statement) (string, error) {
	builder := NewOutputBuilder()
...
func Translated(translatedStatements []translate.TranslatedStatement) (string, error) {
	builder := NewOutputBuilder()

	for idx, translatedStatement := range translatedStatements {
		if idx > 0 {
			builder.Write("\n")
		}

		if statement, err := Statement(translatedStatement.Statement); err != nil {
			return "", err
		} else {
